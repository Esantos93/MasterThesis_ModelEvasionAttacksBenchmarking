...
def main():
    ...
    # 2. Select PCAP
    pcaps = [f for f in os.listdir(PCAP_DIR) if f.endswith(('.pcap', '.pcapng'))]
    pcaps.sort()
    ...
    # 3. Select Ruleset
    rules = [f for f in os.listdir(RULES_DIR) if f.endswith('.rules')]
    rules.sort()
    # Add option 0 manually
    print("\nStep 2: Select the Ruleset file:")
    print("0) No Ruleset (Run only built-in inspectors)")
    ...
    if full_rule_path:
        lua_ips_config += f", include = '{full_rule_path}'"
    lua_ips_config += " }"
    ...
    try:
        ...
        result = subprocess.run(snort_cmd, check=True)
