...
#This function chooses the first deterministic flow-group key for the current V1 flow-based implementation.
def flow_group_key(packet: dict[str, Any]) -> str:
    ...
    if len(candidate_flow_ids) == 1:
        return f"candidate_{candidate_flow_ids[0]}"
...
def group_flow_based(records: list[Any]) -> list[dict[str, Any]]:
    ...
    for group_index, key in enumerate(sorted(groups_by_key), start=1):
        ...
        parent_group_id = safe_key if safe_key.startswith(("flow_", "candidate_", "unresolved_")) else f"flow_{safe_key}"
        groups.append(
            {
...
def group_records_by_policy(*, records: list[Any], grouping_policy: str, group_size_packets: int) -> list[dict[str, Any]]:
...
def policy_output_subdir(grouping_policy: str, group_size_packets: int) -> str:
    if grouping_policy == "fixed_packet_count":
...
def build_group_metadata(parent_group_id: str, group_index: int, records: list[dict[str, Any]], grouping_policy: str, group_size_packets: int, unit_type: str) -> dict[str, Any]:
...
def build_prompt_units_for_group(
    *,
    ...
    group_metadata = build_group_metadata(parent_group_id, group_index, records, grouping_policy, group_size_packets, unit_type)
    base_packets = [
